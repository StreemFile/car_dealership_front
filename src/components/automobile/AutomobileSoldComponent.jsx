...
const AutomobileSoldComponent = (props) => {
    ...
    useEffect(() => {
        AutomobileService.getAll().then(result => {
            setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.expectedSellingPrice > b.expectedSellingPrice ? 1 : -1));
        })
    }, [])

    const handleSort = (event) => {
        if (event.target.value === "Зростаюча ціна") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.expectedSellingPrice > b.expectedSellingPrice ? 1 : -1)))
        }
        if (event.target.value === "Спадаюча ціна") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.expectedSellingPrice > b.expectedSellingPrice ? -1 : 1)))
        }
        if (event.target.value === "Марка") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.modelAndPackage.make.name > b.modelAndPackage.make.name ? -1 : 1)))
        }
        if (event.target.value === "Модель") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.modelAndPackage.model > b.modelAndPackage.model ? -1 : 1)))
        }
        if (event.target.value === "Зростаючий рік випуску") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.manufactureYear > b.manufactureYear ? 1 : -1)))
        }
        if (event.target.value === "Спадаючий рік випуску") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.manufactureYear > b.manufactureYear ? -1 : 1)))
        }
        if (event.target.value === "Зростаюча кубатура") {
            return AutomobileService.getAll().then(result => setAutomobiles(result.data.filter(item => item.availability === "Проданий").sort((a, b) => a.engine.cubicCapacity > b.engine.cubicCapacity ? 1 : -1)))
        }
...
export default AutomobileSoldComponent;