...
const AutomobileEditComponent = (props) => {
    const {id} = useParams()
    const [automobile, setAutomobile] = useState(null);
    const [automobileInOrder, setAutomobileInOrder] = useState(null);
    ...
    const [isPackDisabled, setIsPackDisabled] = useState(false);
    ...
    useEffect(() => {
        ...
        if (automobile !== null) {
            setVehicleType(automobile.vehicleType.name);
            setExportedFrom(automobile.exportedFrom.name);
            setMake(automobile.modelAndPackage.make.name)
            setModel(automobile.modelAndPackage.model);
            setPack(automobile.modelAndPackage.pack);
            setDrive(automobile.drive);
            setFuelType(automobile.engine.fuelType)
            setTransmissionType(automobile.engine.transmissionType)
            setCubicCapacity(automobile.engine.cubicCapacity)
            setPowerPS(automobile.engine.powerPS)
            setPowerKW(automobile.engine.powerKW)
            setExteriorColor(automobile.exteriorColor.name)
            setInteriorColor(automobile.interiorColor.name)
            let parkSens = parkingSensors.map(item => {
                if (automobile.parkingSensors.includes(item.value)) {
                    item.isChosen = true
                }
                return item
            })
            setParkingSensors(parkSens)
            let sec = securities.map(item => {
                if (automobile.securities.includes(item.value)) {
                    item.isChosen = true
                }
                return item
            })
            setSecurities(sec)
            setHeadlightsType(automobile.headlightsType)
            let taw = tyresAndWheels.map(item => {
                if (automobile.tyresAndWheels.includes(item.value)) {
                    item.isChosen = true
                }
                return item
            })
            setTyresAndWheels(taw)
            setNumberOfDoors(automobile.numberOfDoors)
            setNumberOfSeats(automobile.numberOfSeats)
            setInteriorMaterial(automobile.interiorMaterial)
            let intFeat = interiorFeatures.map(item => {
                if (automobile.interiorFeatures.includes(item.value)) {
                    item.isChosen = true
                }
                return item
            })
            setInteriorFeatures(intFeat)
            let ext = extras.map(item => {
                if (automobile.extras.includes(item.value)) {
                    item.isChosen = true
                }
                return item
            })
            setExtras(ext)
            setMileage(automobile.mileage)
            setManufactureYear(automobile.manufactureYear)
            setVin(automobile.vin)
            setIsAvailable(automobile.availability === "В наявності")
            setActualPrice(automobile.actualPrice)
            setExpectedSellingPrice(automobile.expectedSellingPrice)
            setDealership(automobile.dealership)
                                ...
                                <AutomobileInfo>
                                    ...
                                    <div>
                                        ...
                                        <label> Матеріал салону
                                            ...
                                            >
                                                <option selected="selected" style={{display: "none"}}>{interiorMaterial}
                                                </option>
...
export default AutomobileEditComponent;